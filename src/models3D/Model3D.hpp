...
class Model3D {
  ...
  // apply to Quad
  virtual void
  applyTo(std::vector<Quad>& quads) const = 0;
  ...
  static bool
  inplaneScope(const Mat2X& szMesh,
      bool checkR,
      double minR,
      double maxR,
      bool checkZ,
    ...
    // check R
    if (checkR) {
      if (crdModel.row(0).maxCoeff() < minR || crdModel.row(0).minCoeff() > maxR) {
        return false;
      }
    }
    ...
    if (checkZ) {
      // radius -> depth
      if (useDepth) {
        double router = (depthSolid ? geodesy::getOuterSolidRadius() : geodesy::getOuterRadius());
        crdModel.row(1) = router - crdModel.row(1).array();
      }
      if (crdModel.row(1).maxCoeff() < minZ || crdModel.row(1).minCoeff() > maxZ) {
        return false;
      }
    }
    return true;
  ...
  coordsFromMeshToModel(const eigen::DMatX3& spzMesh,
    ...
    if (sourceCentered) {
      if (geodesy::isCartesian()) {
        ...
        // (s, phi) to (x, y)
        if (xy) {
          crdModel.col(0) = spzMesh.col(0).array() * spzMesh.col(1).array().cos();
          crdModel.col(1) = spzMesh.col(0).array() * spzMesh.col(1).array().sin();
        }
      } else {
        // spz -> RTZ
      ...
      if (geodesy::isCartesian()) {
        ...
        if ((spzMesh.array().col(2) < spzMesh.array().col(0) * 10.).any()) {
          throw std::runtime_error("Model3D::coordsFromMeshToModel || "
