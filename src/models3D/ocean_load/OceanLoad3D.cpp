...
// apply to Quad
void
OceanLoad3D::applyTo(std::vector<Quad>& quads) const {
  if (!isSuperOnly()) {
    for (Quad& quad : quads) {
      // check surface
...
OceanLoad3D::buildInparam(const ExodusMesh& exodusMesh,
  ...
  if (className == "StructuredGridO3D") {
    ...
    // construct
    return std::make_shared<const StructuredGridO3D>(modelName,
        fname,
        crdVarNames,
        shuffleData,
        sourceCentered,
