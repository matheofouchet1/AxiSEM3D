...
// constructor
StructuredGridV3D::StructuredGridV3D(const std::string& modelName,
    ...
    mDepthSolid(depthSolid), mUndulatedGeometry(undulated), mElementCenter(center),
    mSuperOnly(superOnly) {
  // read info
  std::transform(propertyInfo.begin(),
      propertyInfo.end(),
  ...
  if (mUndulatedGeometry && mUseDepth) {
    throw std::runtime_error("StructuredGridV3D::StructuredGridV3D || "
  ...
  for (const std::string& key : mPropertyKeys) {
    if (key.front() == 'C') {
      ...
      const std::vector<std::string>& allCIJ = {"C11",
          "C12",
          "C13",
          "C14",
          "C15",
      ...
      for (const std::string& CIJ : allCIJ) {
        ...
        int i = bstring::cast<int>(CIJ.substr(1, 1), "StructuredGridV3D::StructuredGridV3D") - 1;
        int j = bstring::cast<int>(CIJ.substr(2, 1), "StructuredGridV3D::StructuredGridV3D") - 1;
  ...
  auto initGrid = [this, &dataInfo, &shuffleData, &lengthUnit, &angleUnit]() {
    // grid
    mGrid =
        std::make_unique<StructuredGrid<3, double>>(mFileName, mCrdVarNames, dataInfo, shuffleData);
    // coordinate units
    sg_tools::constructUnits(*mGrid, mSourceCentered, mXY, true, lengthUnit, angleUnit);
    // longitude range
    if (!mSourceCentered) {
...
// get property info
void
StructuredGridV3D::getPropertyInfo(
...
StructuredGridV3D::getProperties(const eigen::DMatX3& spz,
  ...
  for (int ipnt = 0; ipnt < nCardinals; ipnt++) {
    ...
    if (val(0) > err * .9) {
      ...
      if (mElementCenter) {
        if ((mSourceCentered && (!mXY) && dimOutOfRange == 0) || dimOutOfRange == 2) {
          throw std::runtime_error("StructuredGridV3D::getProperties || "
              ...
              bstring::toString(crdGrid(ipnt, dimOutOfRange)) + "|| Out-of-range dimension = " +
              bstring::toString(dimOutOfRange) + "|| Model name = " + mModelName);
        }
      }
    }
  }
...
// verbose
std::string
StructuredGridV3D::verbose() const {
  if (!mpi::root()) {
  ...
  if (mUseDepth) {
    ...
    ss << boxEquals(4, 29, "radial geometry", mUndulatedGeometry ? "undulated" : "reference");
  }
  ...
  for (int iprop = 0; iprop < mPropertyKeys.size(); iprop++) {
    ...
    ss << std::setw(8) << sReferenceKindStr.at(mPropertyReferenceKinds[iprop]) << " | ";
