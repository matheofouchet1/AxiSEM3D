...
namespace sg_tools {
  ...
  // horizontal coords
  inline void
  inparamHorizontal(const InparamYAML& gm,
      const std::string& rootc,
  ...
  // vertical coords
  inline void
  inparamVertical(const InparamYAML& gm,
      const std::string& rootc,
      const std::string& modelName,
      const std::string& className,
      bool& useDepth,
    ...
    if (type == "DEPTH") {
      ...
      throw std::runtime_error("sg_tools::inparamVertical || "
  ...
  template <int NDIM>
  void
  inparamVarRank(const InparamYAML& gm,
      const std::string& rootc,
      const std::string& modelName,
      const std::string& className,
      std::array<std::string, NDIM>& crdVarNames,
      std::array<int, NDIM>& shuffleData) {
    const std::vector<std::string>& vars = gm.getVector<std::string>(rootc + ":nc_variables");
    const std::vector<int>& rank = gm.getVector<int>(rootc + ":data_rank");
    if (vars.size() != NDIM || rank.size() != NDIM) {
      throw std::runtime_error("sg_tools::inparamVarRank || "
                               "The size of coordinates:nc_variables and "
                               "coordinates:data_rank must be " +
          bstring::toString(NDIM) +
          "."
          " || Model name: " +
          modelName + " || Class name: " + className);
    }
  ...
  // coord units
  inline void
  inparamUnits(const InparamYAML& gm,
      const std::string& rootc,
      bool xy,
      double& lengthUnit,
      double& angleUnit) {
    lengthUnit = gm.getWithOptions<double>(rootc + ":length_unit", {{"m", 1.}, {"km", 1e3}});
    if (!xy) {
      angleUnit = gm.getWithLimits<double>(
          rootc + ":angle_unit", {{"degree", numerical::dDegree}, {"radian", 1.}});
    }
  }
  ...
  constructUnits(Grid& grid,
    ...
    auto vectorTimesFactor = [](std::vector<double>& vec, double factor) {
      std::transform(
          vec.begin(), vec.end(), vec.begin(), [factor](double x) -> double { return factor * x; });
    };
  ...
  template <class Grid>
  bool
  constructLon360(const Grid& grid, const std::string& modelName) {
    const std::vector<double>& lon = grid.getGridCoords()[1];
    if (lon.front() < 0. - numerical::dEpsilon && lon.back() > 180. + numerical::dEpsilon) {
      throw std::runtime_error("sg_tools::constructLon360 || "
  ...
  inline std::string
  verboseHead(
      const std::string& modelName, const std::string& className, const std::string& fileName) {
    std::stringstream ss;
  ...
  // coords
  inline std::string
  verboseCoords(bool sourceCentered,
      bool xy,
      bool vertical,
      bool useDepth,
