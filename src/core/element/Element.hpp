...
class Element {
  public:
  // constructor
  Element(int quadTag, std::unique_ptr<const GradQuad>& gq, std::unique_ptr<const PRT>& prt) :
  ...
  Element(const Element& other) :
      mQuadTag(other.mQuadTag), mGradQuad(std::make_unique<GradQuad>(*(other.mGradQuad))),
      mPRT(other.mPRT == nullptr ? nullptr : std::make_unique<PRT>(*(other.mPRT))) {
    mTransform = nullptr;
  }
  ...
  // get point
  virtual Point&
  getPoint(int ipnt) const = 0;

  // point set
  void
  pointSet(bool elemInFourier);
