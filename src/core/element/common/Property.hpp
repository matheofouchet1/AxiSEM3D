...
template <int P, int N = P * P> class Property {
  ...
  Property(const DMatPP_RM& data1D) :
      mData1D(std::make_unique<RMatPP_RM>(data1D.template cast<numerical::Real>())) {
    // nothing
  }

  // 3D constructor
  Property(const DMatXN& data3D) : mData3D(data3D.template cast<numerical::Real>()) {
    // nothing
  }

  // copy constructor
  Property(const Property& other) :
      mData1D((other.mData1D == nullptr) ? nullptr : std::make_unique<RMatPP_RM>(*(other.mData1D))),
  ...
  // check compatibility
  void
  checkCompatibility(bool isProp1D,
      int nr,
      bool elemInFourier,
  ...
  // expand workspace
  void
  expandWorkspace(bool isProp1D, int nr) {
    if (isProp1D) {
      if (mData1D == nullptr) {
  ...
  // this = other * factor
  void
  set(bool isProp1D, const Property& other, numerical::Real factor) {
    if (isProp1D) {
  ...
  // this = other * factor
  void
  set(bool isProp1D, const Property& other, const Property& factor) {
    if (isProp1D) {
      *mData1D = other.mData1D->cwiseProduct(*(factor.mData1D));
    } else {
      mData3D.topRows(other.mData3D.rows()) = other.mData3D.cwiseProduct(factor.mData3D);
    }
  }
