...
template <int PED, int N = PED * PED> class FieldArithmetic {
  ...
  ///////////////// R = F * P /////////////////
  // CASE == _1D_FR
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_FR, void>::type
  FxP(int nx, TypeR& R, int dimR, const TypeF& F, int dimF, const PropertyN& P) {
    R[nx][dimR] = F[nx][dimF].cwiseProduct(P.get1D());
  }
  // CASE == _1D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
  FxP(int nx, TypeR& R, int dimR, const TypeF& F, int dimF, const PropertyN& P) {
    R.blockN(dimR, nx) = F.blockN(dimF, nx) * flattenDiag(P.get1D());
  }
  // CASE == _3D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_3D_CD, void>::type
  FxP(int nx, TypeR& R, int dimR, const TypeF& F, int dimF, const PropertyN& P) {
  ...
  F1xP1_F2xP2(int nx,
      ...
      int dimF2,
      const PropertyN& P2) {
    R[nx][dimR] = (F[nx][dimF1].cwiseProduct(P1.get1D()) + F[nx][dimF2].cwiseProduct(P2.get1D()));
  }
  // CASE == _1D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
  ...
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_FR, void>::type
  R1_FxP(int nx, TypeR& R, int dimR, int dimR1, const TypeF& F, int dimF, const PropertyN& P) {
  ...
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
  R1_FxP(int nx, TypeR& R, int dimR, int dimR1, const TypeF& F, int dimF, const PropertyN& P) {
    R.blockN(dimR, nx) = (R.blockN(dimR1, nx) + F.blockN(dimF, nx) * flattenDiag(P.get1D()));
  }
  // CASE == _3D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_3D_CD, void>::type
  R1_FxP(int nx, TypeR& R, int dimR, int dimR1, const TypeF& F, int dimF, const PropertyN& P) {
    R.blockN(dimR, nx) = (R.blockN(dimR1, nx) + F.blockN(dimF, nx).cwiseProduct(P.get3D()));
  }
  ...
  F123xP(int nx,
      ...
      int dimF3,
      const PropertyN& P) {
    R[nx][dimR] = (F[nx][dimF1] + F[nx][dimF2] + F[nx][dimF3]).cwiseProduct(P.get1D());
  }
  // CASE == _1D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
  F123xP(int nx,
      ...
      const PropertyN& P) {
    R.blockN(dimR, nx) =
        (F.blockN(dimF1, nx) + F.blockN(dimF2, nx) + F.blockN(dimF3, nx)) * flattenDiag(P.get1D());
  }
  // CASE == _3D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_3D_CD, void>::type
  F123xP(int nx,
      ...
      const PropertyN& P) {
    R.blockN(dimR, nx) =
        (F.blockN(dimF1, nx) + F.blockN(dimF2, nx) + F.blockN(dimF3, nx)).cwiseProduct(P.get3D());
  }
  ...
  F1abxP1_F2xP2_F3xP3(int nx,
      ...
      int dimF3,
      const PropertyN& P3) {
    R[nx][dimR] = ((F[nx][dimF1a] + F[nx][dimF1b]).cwiseProduct(P1.get1D()) +
        F[nx][dimF2].cwiseProduct(P2.get1D()) + F[nx][dimF3].cwiseProduct(P3.get1D()));
  }
  // CASE == _1D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
  F1abxP1_F2xP2_F3xP3(int nx,
      ...
      int dimF3,
      const PropertyN& P3) {
    R.blockN(dimR, nx) = ((F.blockN(dimF1a, nx) + F.blockN(dimF1b, nx)) * flattenDiag(P1.get1D()) +
        F.blockN(dimF2, nx) * flattenDiag(P2.get1D()) +
        F.blockN(dimF3, nx) * flattenDiag(P3.get1D()));
  }
  // CASE == _3D_CD
  ...
  F1abxP1_F2xP2_F3xP3(int nx,
      ...
      int dimF3,
      const PropertyN& P3) {
    R.blockN(dimR, nx) = ((F.blockN(dimF1a, nx) + F.blockN(dimF1b, nx)).cwiseProduct(P1.get3D()) +
        F.blockN(dimF2, nx).cwiseProduct(P2.get3D()) +
        F.blockN(dimF3, nx).cwiseProduct(P3.get3D()));
  }
  ...
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_FR, void>::type
  F1_F2(int nx, TypeR& R, int dimR, const TypeF& F, int dimF1, int dimF2) {
  ...
  R1xP1_F2xP2(int nx,
      ...
      int dimF2,
      const PropertyN& P2) {
    R[nx][dimR] = (R[nx][dimR1].cwiseProduct(P1.get1D()) + F[nx][dimF2].cwiseProduct(P2.get1D()));
  }
  // CASE == _1D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
  ...
  F1xP1_to_F6xP6(int nx,
      ...
      const PropertyN& P6) {
    R[nx][dimR] = (F[nx][dimF1].cwiseProduct(P1.get1D()) + F[nx][dimF2].cwiseProduct(P2.get1D()) +
        F[nx][dimF3].cwiseProduct(P3.get1D()) + F[nx][dimF4].cwiseProduct(P4.get1D()) +
        F[nx][dimF5].cwiseProduct(P5.get1D()) + F[nx][dimF6].cwiseProduct(P6.get1D()));
  }
  // CASE == _1D_CD
  template <CaseFA CASE, class TypeR, class TypeF>
  static inline typename std::enable_if<CASE == CaseFA::_1D_CD, void>::type
