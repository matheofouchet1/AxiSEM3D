...
// construct derived
template <typename T, int ndim, int nCols>
void
...
SourceTimeFunctionNetCDF<T, ndim, nCols>::loadNextBufferChunk() {
  ...
  // read time
  mReader->readVariable(mTimeID, "time_points", mTimePoints, {mTimeStepOfPattern0}, {readCount});

  // read data
  mReader->readVariable(mPatternReID,
      mVariableName + "_RE",
      sReadRe,
      {mTimeStepOfPattern0, 0},
      {readCount, mNu_1 * nCols});
  mReader->readVariable(mPatternImID,
      mVariableName + "_IM",
      sReadIm,
      {mTimeStepOfPattern0, 0},
      {readCount, mNu_1 * nCols});
  ...
  for (int itime = 0; itime < readCount; itime++) {
    for (int alpha = 0; alpha < mNu_1; alpha++) {
      mPatterns[itime].row(alpha).real() = sReadRe.block(0, pos, 1, nCols);
      mPatterns[itime].row(alpha).imag() = sReadIm.block(0, pos, 1, nCols);
      pos += nCols;
    }
  }
}
