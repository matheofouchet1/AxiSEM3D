...
template <typename T, int ndim, int nCols = ndim * spectral::nPEM> class SourceTimeFunctionNetCDF {
  public:
  // pattern matrix
  ...
  // constructor
  SourceTimeFunctionNetCDF(bool alignedToTimeStep,
      int bufferSize,
      const std::shared_ptr<NetCDF_Reader>& reader,
      const std::string& variableName) :
      mAlignedToTimeStep(alignedToTimeStep), mReader(reader), mVariableName(variableName) {
    // construct derived
  ...
  // get order
  int
  getPatternNu_1() const {
    return mNu_1;
  }

  // get value at a time step
  void
  getPatternAtTimeStep(int timeStep, double time, CTMatXND& result) {
    if (mAlignedToTimeStep) {
      ...
      // retrieve by index
      result.topRows(mNu_1) = mPatterns[timeStep - mTimeStepOfPattern0];
    } else {
      // check buffer
      while (time > mTimePoints[mTimeStepOfPatternLast - mTimeStepOfPattern0]) {
        loadNextBufferChunk();
      }
      ...
      try {
        vector_tools::linearInterpSorted(mTimePoints,
            time,
            index0,
            index1,
            factor0,
            factor1,
            0,
            mTimeStepOfPatternLast - mTimeStepOfPattern0 + 1);
      } catch (...) {
        throw std::runtime_error("SourceTimeFunctionNetCDF::getPatternAtTimeStep || "
                                 "Target time point is out of range.");
      }

      result.topRows(mNu_1) = (mPatterns[index0] * (T)factor0 + mPatterns[index1] * (T)factor1);
    }
  }
  ...
  // construct derived
  void
  constructDerived(int bufferSize);
