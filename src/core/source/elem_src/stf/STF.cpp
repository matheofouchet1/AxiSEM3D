...
// build from inparam
std::unique_ptr<STF>
STF::buildInparam(int sindex, const std::string& sourceName, double dt) {
  // short alias
  const InparamYAML& gm = inparam::gInparamSource;
  const std::string& root = ("list_of_sources:[" + bstring::toString(sindex) + "]:" + sourceName +
  ...
  if (className == "GaussianSTF") {
    ...
    int order = gm.getWithLimits<int>(root + ":use_derivative_integral",
        {{"ERF", -1}, {"GAUSSIAN", 0}, {"FIRST_DERIVATIVE", 1}, {"RICKER", 2}});
    ...
    if (className == "StreamSTF") {
      const std::string& fname = gm.get<std::string>(root + ":ascii_data_file");
      return std::make_unique<StreamSTF>(fname, pm, left, right);
    } else {
      const std::string& fname = gm.get<std::string>(root + ":nc_data_file");
      const std::string& varTime = gm.get<std::string>(root + ":nc_var_times");
      const std::string& varData = gm.get<std::string>(root + ":nc_var_data");
      int chunkSize =
          gm.getWithOptions<int>(root + ":chunk_size", {{"NONE", std::numeric_limits<int>::max()}});
      return std::make_unique<NetCDF_STF>(fname, varTime, varData, chunkSize, pm, left, right);
    }
  } else {
    throw std::runtime_error("STF::buildInparam || "
                             "Unknown class of source-time function."
                             " || Source name: " +
        sourceName + " || STF class name: " + className);
  }
}
