...
// initialize
void
ElementIO_ParNetCDF::initialize(const std::string& groupName,
    int numRecordSteps,
  ...
  // base
  ElementIO::initialize(
      groupName, numRecordSteps, channels, npnts, naGrid, elemNaInfoL, elemCoordsL);
  ...
  if (mpi::rank() == mRankWithMaxNumElements) {
    ...
    // first element index on na-grid
    firstElemNaGridRanks.assign(mpi::nproc(), std::vector<int>(naGrid.size(), 0));
    // loop over ranks
    for (int irank = 1; irank < mpi::nproc(); irank++) {
      ...
      for (int inag = 0; inag < naGrid.size(); inag++) {
        firstElemNaGridRanks[irank][inag] += firstElemNaGridRanks[irank - 1][inag];
      }
    }
    ...
    for (int irank = 0; irank < mpi::nproc(); irank++) {
      ...
      for (int ielem = 0; ielem < nelocal; ielem++) {
        ...
        int nagIndex = naGridIndexDict[nag];
        elemNaInfoAll(row, 4) = elemNaInfoAll(row, 3) + firstElemNaGridRanks[irank][nagIndex];
        row++;
      }
    }
    ...
    // na-grid
    mNcFile->defineVariable("list_na_grid", {{"dim_na_grid", naGrid.size()}}, (int)-1);

    // element-na info
    mNcFile->defineVariable("list_element_na", {{"dim_element", nelem}, {"dim_5", 5}}, (int)-1);
    ...
    for (int inag = 0; inag < naGrid.size(); inag++) {
      const std::string& strNag = bstring::toString(naGrid[inag]);
      mNcFile->writeWholeVariable("list_element__NaG=" + strNag, elemsNaGrid[inag]);
    }
  ...
  mpi::scatter(firstElemNaGridRanks, mFirstElemNaGrid, mRankWithMaxNumElements);
}

// finalize
void
ElementIO_ParNetCDF::finalize() {
  // close files
  if (mNcFile) {
    mNcFile->close();
