...
// set in group
void
ElementOpFluid::setInGroup(int dumpIntv, const channel::fluid::ChannelOptions& chops, int nphis) {
  // sizes
  int na = getNa(nphis);
...
ElementOpFluid::record(int bufferLine,
  ...
  if (chops.mNeedBufferX) {
    ...
    recordToElem<1>(sCXXN1, nu_1, sRXXN1, expIAlphaPhi, mBufferX, bufferLine);
  }
  if (chops.mNeedBufferU) {
  ...
  if (chops.mNeedBufferP) {
    ...
    recordToElem<1>(sCPXN1, nu_1, sRPXN1, expIAlphaPhi, mBufferP, bufferLine);
  }
  if (chops.mNeedBufferD) {
    mElement->getDeltaField(sCDXN1);
    recordToElem<1>(sCDXN1, nu_1, sRDXN1, expIAlphaPhi, mBufferD, bufferLine);
  }
}
...
void
ElementOpFluid::processReport(int bufferLine,
    const channel::fluid::ChannelOptions& chops,
    int elemIndexNaGrid,
    int naGridIndex,
  ...
  for (int ich = 0; ich < chops.mStdChannels.size(); ich++) {
    ...
    if (ftype == channel::fluid::FieldType::Chi) {
      dumpToIO<1>(mBufferX, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
      ...
      dumpToIO<1>(mBufferP, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else if (ftype == channel::fluid::FieldType::Delta) {
      dumpToIO<1>(mBufferD, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else {
