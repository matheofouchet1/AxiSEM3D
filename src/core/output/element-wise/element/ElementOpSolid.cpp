...
// set in group
void
ElementOpSolid::setInGroup(int dumpIntv, const channel::solid::ChannelOptions& chops, int nphis) {
  // sizes
  int na = getNa(nphis);
...
ElementOpSolid::record(int bufferLine,
  ...
  if (chops.mNeedBufferU) {
    ...
    recordToElem<3>(sCUXN3, nu_1, sRUXN3, expIAlphaPhi, mBufferU, bufferLine);
  }
  if (chops.mNeedBufferG) {
    mElement->getNablaField(sCGXN9, needRTZ);
    recordToElem<9>(sCGXN9, nu_1, sRGXN9, expIAlphaPhi, mBufferG, bufferLine);
  }
  if (chops.mNeedBufferE) {
    mElement->getStrainField(sCEXN6, needRTZ);
    recordToElem<6>(sCEXN6, nu_1, sREXN6, expIAlphaPhi, mBufferE, bufferLine);
  }
  if (chops.mNeedBufferR) {
    mElement->getCurlField(sCRXN3, needRTZ);
    recordToElem<3>(sCRXN3, nu_1, sRRXN3, expIAlphaPhi, mBufferR, bufferLine);
  }
  if (chops.mNeedBufferS) {
    mElement->getStressField(sCSXN6, needRTZ);
    recordToElem<6>(sCSXN6, nu_1, sRSXN6, expIAlphaPhi, mBufferS, bufferLine);
  }
}
...
void
ElementOpSolid::processReport(int bufferLine,
    const channel::solid::ChannelOptions& chops,
    int elemIndexNaGrid,
    int naGridIndex,
  ...
  for (int ich = 0; ich < chops.mStdChannels.size(); ich++) {
    ...
    if (ftype == channel::solid::FieldType::Displ) {
      dumpToIO<3>(mBufferU, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else if (ftype == channel::solid::FieldType::Nabla) {
      dumpToIO<9>(mBufferG, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else if (ftype == channel::solid::FieldType::Strain) {
      dumpToIO<6>(mBufferE, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else if (ftype == channel::solid::FieldType::Curl) {
      dumpToIO<3>(mBufferR, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else if (ftype == channel::solid::FieldType::Stress) {
      dumpToIO<6>(mBufferS, fieldIndex, bufferLine, ich, elemIndexNaGrid, naGridIndex, ioBuffers);
    } else {
