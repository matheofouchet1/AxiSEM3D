...
// set in group
void
StationSolid::setInGroup(int dumpIntv, const channel::solid::ChannelOptions& chops) {
  // member buffers
...
// record
void
StationSolid::record(int bufferLine, const channel::solid::ChannelOptions& chops) {
...
StationSolid::processReport(int bufferLine,
  ...
  for (int ich = 0; ich < chops.mStdChannels.size(); ich++) {
    ...
    if (ftype == channel::solid::FieldType::Displ) {
      ...
      computeFeedChannel<9>(mBufferG, fieldIndex, bufferLine, ich, stationIndex, bufferFields);
    } else if (ftype == channel::solid::FieldType::Strain) {
      computeFeedChannel<6>(mBufferE, fieldIndex, bufferLine, ich, stationIndex, bufferFields);
    } else if (ftype == channel::solid::FieldType::Curl) {
      computeFeedChannel<3>(mBufferR, fieldIndex, bufferLine, ich, stationIndex, bufferFields);
    } else if (ftype == channel::solid::FieldType::Stress) {
      computeFeedChannel<6>(mBufferS, fieldIndex, bufferLine, ich, stationIndex, bufferFields);
    } else {
...
// process 1: rotate
void
StationSolid::rotate(int bufferLine, const channel::solid::ChannelOptions& chops) {
  bool cartesian = geodesy::isCartesian();
  if (chops.mNeedBufferU) {
    rotateField<3>(mBufferU, bufferLine, mElement->displInRTZ(), chops.mWCS, cartesian);
  }
  if (chops.mNeedBufferG) {
    rotateField<9>(mBufferG, bufferLine, mElement->nablaInRTZ(), chops.mWCS, cartesian);
  }
  if (chops.mNeedBufferE) {
    // halve off-diagonal components before rotation
    mBufferE.rightCols(3) *= (numerical::Real).5;
    rotateField<6>(mBufferE, bufferLine, mElement->strainInRTZ(), chops.mWCS, cartesian);
    mBufferE.rightCols(3) *= (numerical::Real)2.;
  }
  if (chops.mNeedBufferR) {
    rotateField<3>(mBufferR, bufferLine, mElement->curlInRTZ(), chops.mWCS, cartesian);
  }
  if (chops.mNeedBufferS) {
    rotateField<6>(mBufferS, bufferLine, mElement->stressInRTZ(), chops.mWCS, cartesian);
  }
}
