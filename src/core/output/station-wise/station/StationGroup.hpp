...
///////////////////// type inference /////////////////////
template <class StationT> struct StationInference {};

template <> struct StationInference<StationSolid> {
  typedef channel::solid::ChannelOptions ChannelOptions;
  inline static const auto& gChannelMap = channel::solid::gChannelMap;
};

template <> struct StationInference<StationFluid> {
...
template <class StationT> class StationGroup {
  ...
  // add a station
  void
  addStation(std::unique_ptr<StationT>& station) {
  ...
  initialize() {
    ...
    for (int ich = 0; ich < nch; ich++) {
      const std::string& chstr = std::get<0>(
          StationInference<StationT>::gChannelMap.at(mChannelOptions.mStdChannels[ich]));
  ...
  dumpToFile() {
    ...
    for (int ist = 0; ist < mStations.size(); ist++) {
      mStations[ist]->processReport(mBufferLine, mChannelOptions, ist, mBufferFields);
    }
