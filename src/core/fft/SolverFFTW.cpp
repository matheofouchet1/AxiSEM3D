...
void
SolverFFTW<floatT, HOWMANY>::createPlans(double timeLimitForPlanning) {
  // clear if exists
  clearPlans();
  ...
  new (&mCMatXM) Eigen::Map<CMatXM>(reinterpret_cast<cmplxT*>(cdata), maxNC, HOWMANY);
  ...
  for (const int& NR : mToBeNRs) {
    ...
    // r2c plan
    mPlansR2C.insert({NR,
        InterfaceFFTW<floatT>::planR2C(
            1, &NR, HOWMANY, rdata, NULL, 1, maxNR, cdata, NULL, 1, maxNC, FFTW_MEASURE)});
    // c2r plan
    mPlansC2R.insert({NR,
        InterfaceFFTW<floatT>::planC2R(
            1, &NR, HOWMANY, cdata, NULL, 1, maxNC, rdata, NULL, 1, maxNR, FFTW_MEASURE)});
  }
}
...
template <typename floatT, int HOWMANY>
void
SolverFFTW<floatT, HOWMANY>::clearPlans() {
  // empty
  if (mPlansR2C.size() == 0) {
    return;
  }
...
template <typename floatT, int HOWMANY>
double
SolverFFTW<floatT, HOWMANY>::timeFactorForPlanning() const {
  // empty
  if (mToBeNRs.size() == 0) {
    return 0.;
  }
