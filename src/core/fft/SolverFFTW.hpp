...
///////////////////// fftw3 interface /////////////////////
template <typename floatT> struct InterfaceFFTW {};

template <> struct InterfaceFFTW<double> {
...
template <> struct InterfaceFFTW<float> {
...
template <typename floatT, int HOWMANY> class SolverFFTW {
  //////////////////// buffer type ////////////////////
  ...
  getPlannedNRs() const {
    ...
    std::transform(
        mPlansR2C.begin(), mPlansR2C.end(), std::back_inserter(plannedNRs), [](auto pair) {
          return pair.first;
        });
    return plannedNRs;
  }
  ...
  void
  computeR2C(const RMatXM& r_in, CMatXM& c_out, int NR) {
  ...
  void
  computeR2C(const RMatXM& r_in, vec_arD_MatPP_RM& c_out, int NR) {
  ...
  void
  computeC2R(const CMatXM& c_in, RMatXM& r_out, int NR) {
    // feed input
    int NC = NR / 2 + 1;
  ...
  template <typename vec_arD_MatPP_RM>
  void
  computeC2R(const vec_arD_MatPP_RM& c_in, RMatXM& r_out, int NR) {
    // feed input
    int NC = NR / 2 + 1;
