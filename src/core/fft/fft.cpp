...
namespace fft {
  ...
  createPlans(double timeLimitForPlanning) {
    ...
    double tfSum = std::max(tf1 + tf3 + tfN3 + tfN6 + tfN9, numerical::dEpsilon);
  ...
  verbose(const std::string& stageKey) {
    ...
    ss << internal::verbose("FFT on GLL points, \"howmany\" = 1", gFFT_1.getPlannedNRs(), 1);
    ss << internal::verbose("FFT on GLL points, \"howmany\" = 3", gFFT_3.getPlannedNRs(), 3);
    ss << internal::verbose("FFT on elements, \"howmany\" = N3", gFFT_N3.getPlannedNRs(), nPEM * 3);
    ss << internal::verbose("FFT on elements, \"howmany\" = N6", gFFT_N6.getPlannedNRs(), nPEM * 6);
    ss << internal::verbose("FFT on elements, \"howmany\" = N9", gFFT_N9.getPlannedNRs(), nPEM * 9);
    ss << boxBaseline() << "\n\n";
    return ss.str();
  }
  ...
  namespace internal {
    ...
    diagnosticInfo(const std::vector<int>& plannedNRs, int HOWMANY) {
      ...
      statistics(plannedNRs, HOWMANY, numNR_MaxMPI, maxNR_MaxMPI, sumNR_MaxMPI, memGB_MaxMPI);

      // write diagnostic info
      using timer::gPreloopTimer;
      gPreloopTimer.message("HOWMANY = " + bstring::toString(HOWMANY));
      gPreloopTimer.message("# NR's (max over MPI ranks) = " + bstring::toString(numNR_MaxMPI));
      gPreloopTimer.message("max NR (max over MPI ranks) = " + bstring::toString(maxNR_MaxMPI));
      gPreloopTimer.message("sum NR (max over MPI ranks) = " + bstring::toString(sumNR_MaxMPI));
