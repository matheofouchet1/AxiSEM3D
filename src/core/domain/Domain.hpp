...
class StationSolid;
class StationFluid;
template <class StationT> class StationGroup;
...
template <class ElementT> class ElementOpGroup;
...
class Domain {
  ...
  // add a solid point
  void
  addSolidPoint(const std::shared_ptr<SolidPoint>& point);

  // add a fluid point
  void
  addFluidPoint(const std::shared_ptr<FluidPoint>& point);

  // add a solid element
  void
  addSolidElement(const std::shared_ptr<SolidElement>& element);

  // add a fluid element
  void
  addFluidElement(const std::shared_ptr<FluidElement>& element);

  // replace a solid element
  void
  replaceSolidElement(const std::shared_ptr<SolidElement>& element);

  // replace a fluid element
  void
  replaceFluidElement(const std::shared_ptr<FluidElement>& element);

  // set mpi messaging
  void
  setMessaging(std::unique_ptr<Messaging>& msg);

  // verbose
  std::string
  verbose(const std::string& title) const;
  ...
  inline std::unique_ptr<SolidFluidBoundary>&
  getSolidFluidBoundary() {
    return mSolidFluidBoundary;
  }

  // get absorbing boundary
  inline std::unique_ptr<AbsorbingBoundary>&
  getAbsorbingBoundary() {
    return mAbsorbingBoundary;
  }

  // get axial boundary
  inline std::unique_ptr<AxialBoundary>&
  getAxialBoundary() {
    return mAxialBoundary;
  }

  // get fluid surface boundary
  inline std::unique_ptr<FluidSurfaceBoundary>&
  getFluidSurfaceBoundary() {
  ...
  // apply sources
  void
  applySources(int tstep, double time) const;
  ...
  // * solid-fluild
  void
  applyBC_BeforeAssemblingStiff() const;
  ...
  // * fluid surface
  void
  applyBC_AfterAssemblingStiff() const;

  // boundary conditions after computing acceleration
  // * sponge ABC
  void
  applyBC_AfterComputingAccel() const;

  // stiff to accel on points
  void
  computeStiffToAccel() const;

  // mpi phase 1: commGatherSendRecv
  void
  mpiGatherSendRecv() const;

  // mpi phase 2: commWaitScatter
  void
  mpiWaitScatter() const;

  // initialize output
  void
  initializeOutput() const;

  // record output
  void
  recordOutput(int tstep, double time) const;
  ...
  // finalize output
  void
  finalizeOutput() const;

  // do scanning
  void
  doScanning(int tstep) const;

  // report scanning
  void
  reportScanning() const;
