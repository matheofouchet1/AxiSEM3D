...
// add a solid point
void
Domain::addSolidPoint(const std::shared_ptr<SolidPoint>& point) {
...
// add a fluid point
void
Domain::addFluidPoint(const std::shared_ptr<FluidPoint>& point) {
...
// add a solid element
void
Domain::addSolidElement(const std::shared_ptr<SolidElement>& element) {
...
// add a fluid element
void
Domain::addFluidElement(const std::shared_ptr<FluidElement>& element) {
...
void
Domain::replaceSolidElement(const std::shared_ptr<SolidElement>& element) {
  // check domain tag
...
void
Domain::replaceFluidElement(const std::shared_ptr<FluidElement>& element) {
  // check domain tag
  int domainTag = element->getDomainTag();
  if (domainTag == -1) {
...
// set mpi messaging
void
Domain::setMessaging(std::unique_ptr<Messaging>& msg) {
  mMessaging = std::move(msg);
}

// verbose
std::string
Domain::verbose(const std::string& title) const {
  //////////////////////////// count info ////////////////////////////
  // point
  ...
  std::map<std::string, int> typeCountSFB = mSolidFluidBoundary->countInfo(*mMessaging);
  mpi::aggregate(typeCountSFB, 0, MPI_SUM);

  // absorbing boundary
  std::map<std::string, int> typeCountABB = mAbsorbingBoundary->countInfo(*mMessaging);
...
void
Domain::setWavefieldInjection(std::unique_ptr<WavefieldInjection>& wj) {
  wj->setInDomain(*this);
  mWavefieldInjection = std::move(wj);
}

void
Domain::addElementSource(std::unique_ptr<const ElementSource>& esrc) {
  mElementSources.push_back(std::move(esrc));
}

// add station group in solid
void
Domain::addStationGroupInSolid(std::unique_ptr<StationGroupInSolid>& stgrp) {
  mStationGroupInSolids.push_back(std::move(stgrp));
}

// add station group in fluid
void
Domain::addStationGroupInFluid(std::unique_ptr<StationGroupInFluid>& stgrp) {
  mStationGroupInFluids.push_back(std::move(stgrp));
}

// add element group in solid
void
Domain::addElementOpGroupInSolid(std::unique_ptr<ElementOpGroupInSolid>& elgrp) {
  mElementOpGroupInSolids.push_back(std::move(elgrp));
}

// add element group in fluid
void
Domain::addElementOpGroupInFluid(std::unique_ptr<ElementOpGroupInFluid>& elgrp) {
  mElementOpGroupInFluids.push_back(std::move(elgrp));
}

// set wavefield scanning
void
Domain::setWavefieldScanning(std::unique_ptr<WavefieldScanning>& ws) {
...
// apply sources
void
Domain::applySources(int tstep, double time) const {
  // wavefield injection
...
// * solid-fluild
void
Domain::applyBC_BeforeAssemblingStiff() const {
...
// * fluid surface
void
Domain::applyBC_AfterAssemblingStiff() const {
  // the following order matters!
...
// * sponge ABC
void
Domain::applyBC_AfterComputingAccel() const {
  mAbsorbingBoundary->applySponge();
}

// stiff to accel on points
void
Domain::computeStiffToAccel() const {
  // solid
...
void
Domain::mpiGatherSendRecv() const {
  mMessaging->commGatherSendRecv();
}

// mpi phase 2: commWaitScatter
void
Domain::mpiWaitScatter() const {
  mMessaging->commWaitScatter();
}

// initialize output
void
Domain::initializeOutput() const {
  // solid stations
  for (const std::unique_ptr<StationGroupInSolid>& stgrp : mStationGroupInSolids) {
    stgrp->initialize();
  }

  // fluid stations
  for (const std::unique_ptr<StationGroupInFluid>& stgrp : mStationGroupInFluids) {
    stgrp->initialize();
  }

  // solid elements
  for (const std::unique_ptr<ElementOpGroupInSolid>& elgrp : mElementOpGroupInSolids) {
    elgrp->initialize();
  }
...
// record output
void
Domain::recordOutput(int tstep, double time) const {
  // solid stations
  for (const std::unique_ptr<StationGroupInSolid>& stgrp : mStationGroupInSolids) {
...
// do scanning
void
Domain::doScanning(int tstep) const {
...
Domain::reportScanning() const {
  ...
  if (mpi::root()) {
    ...
    writer.open(io::gOutputDirectory + "/" + mWavefieldScanning->mFileName, true);
    writer.defModeOn();
    writer.defineVariable("pointwise_sz", {{"dim_point", nPoints}, {"dim_sz", 2}}, numerical::dErr);
    writer.defineVariable("pointwise_Nr", {{"dim_point", nPoints}}, (int)-1);
    writer.defineVariable("starting_Nr_for_scanning", {{"dim_point", nPoints}}, (int)-1);
    writer.defModeOff();
