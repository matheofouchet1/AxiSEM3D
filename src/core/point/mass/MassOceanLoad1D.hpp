...
class MassOceanLoad1D : public Mass {
  ...
  MassOceanLoad1D(double mass, double massOcean, double theta) :
      mIMH_CosT2_p_IMV_SinT2(
          1. / mass * cos(theta) * cos(theta) + 1. / (mass + massOcean) * sin(theta) * sin(theta)),
      mIMH_SinT2_p_IMV_CosT2(
          1. / mass * sin(theta) * sin(theta) + 1. / (mass + massOcean) * cos(theta) * cos(theta)),
      mIMV_m_IMH_x_CosT_SinT((1. / (mass + massOcean) - 1. / mass) * cos(theta) * sin(theta)),
      mIMH(1 / mass) {
    // nothing
  }
