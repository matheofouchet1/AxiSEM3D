...
// verbose begin
void
TimeScheme::verboseBegin(const std::string& tsName) const {
...
// verbose iteration
void
TimeScheme::verboseIter(double elapsed, int tstep, double t) const {
  if (tstep % mVerboseInterval == 0 && io::gVerbose != io::VerboseLevel::None) {
    elapsed /= 3600.;
...
// verbose end
void
TimeScheme::verboseEnd(const std::string& tsName, double elapsed, double t) const {
  if (io::gVerbose != io::VerboseLevel::None) {
    std::stringstream ss;
    ss << io::endl << boxBaseline('T');
...
TimeScheme::reportLoopTimers(
  ...
  // write to file
  if (mpi::root()) {
    const std::string& fname = (io::gOutputDirectory + "/develop/loop_timers.log");
