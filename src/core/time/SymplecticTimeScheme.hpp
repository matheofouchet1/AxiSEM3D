...
class SymplecticTimeScheme : public TimeScheme {
  ...
  template <class Point>
  static void
  update(Point& point, numerical::Real pi_dt, numerical::Real kappa_dt) {
    auto& f = point.getFields();
    // update dt
  ...
  template <class Point>
  void
  launch(const std::vector<std::shared_ptr<Point>>& points) const {
  ...
  template <class Point>
  void
  update(const std::vector<std::shared_ptr<Point>>& points, int iSubIter) const {
  ...
  inline static const std::vector<numerical::Real> sKappa = {(numerical::Real)sAlpha,
