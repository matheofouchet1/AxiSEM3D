...
namespace vector_tools {
  ...
  template <typename T>
  void
  linearInterpSorted(const std::vector<T>& points,
      T value,
      int& index0,
      int& index1,
      T& factor0,
    ...
    if (value < *(itBegin)-tolerance || value > *(itEnd - 1) + tolerance) {
      throw std::runtime_error("vector_tools::linearInterpSorted || "
    ...
    factor1 = ((value - points.at(index0)) / (points.at(index1) - points.at(index0)));
    factor0 = (T)1. - factor1;
  }
  ...
  template <typename T>
  bool
  isSortedUnique(const std::vector<T>& vec) {
    auto adjIter = std::adjacent_find(vec.begin(), vec.end(), std::greater_equal<T>());
  ...
  template <typename T>
  bool
  findSortedUnique(const std::vector<T>& vec, const T& val) {
  ...
  template <typename T>
  size_t
  totalSize(const std::vector<T>& vec) {
    size_t total = 0;
    for (const T& v : vec) {
      total += v.size();
    }
  ...
  template <typename T>
  int
  maxKeyLength(const std::map<std::string, T>& map) {
    int mkl = 0;
  ...
  template <typename T>
  T
  sumValues(const std::map<std::string, T>& map) {
    T sum = (T)0;
    for (auto it = map.begin(); it != map.end(); ++it) {
      sum += it->second;
    }
  ...
  template <typename T>
  void
  aggregate(std::map<std::string, T>& map, const std::string& key, T value) {
    map.insert({key, (T)0});
    map.at(key) += value;
  }
} // namespace vector_tools
