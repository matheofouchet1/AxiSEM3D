...
namespace bstring {
  ...
  split(const std::string& in, const std::string& sep) {
    ...
    boost::split(substrs, orig, boost::is_any_of(sep), boost::token_compress_on);
  ...
  // ****** title ******
  std::string
  boxTitle(const std::string& title, char fill, int length) {
  ...
  // subtitle___________
  std::string
  boxSubTitle(int indent, const std::string& subTitle, char fill, int length) {
    std::stringstream ss;
  ...
  // *******************
  std::string
  boxBaseline(char fill, int length) {
  ...
  boxEquals(int indent,
      ...
      int nspaces,
      int precision) {
    return boxEquals(indent, keyWidth, key, std::string(value), equalSign, nspaces, precision);
  }

  ////////////////////////// error //////////////////////////
  // format warning
  std::string
  warning(const std::string& what, const std::string& head, char fill) {
    // split
  ...
  exception(const std::exception& e) {
    return warning(e.what(), "AxiSEM3D ABORTED UPON RUNTIME EXCEPTION", fmt::gExceptionFill);
  }
} // namespace bstring
