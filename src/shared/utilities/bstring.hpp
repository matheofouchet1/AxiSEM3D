...
namespace bstring {
  ...
  template <typename parType>
  parType
  cast(const std::string& in_str, const std::string& caller) {
    try {
      ...
      throw std::runtime_error("bstring::cast || Error casting string \"" + in_str +
          "\" to type \"" + typeName<parType>() +
          "\" || "
          "Called by " +
          caller);
    }
  ...
  template <typename parType>
  std::string
  toString(const parType& par, int precision = fmt::gFloatPrecision) {
    // use stream
    std::stringstream ss;
  ...
  template <typename T>
  std::string
  range(T min, T max, char fbrac = '[', char bbrac = ']', int precision = fmt::gFloatPrecision) {
    std::stringstream ss;
    ss.precision(precision);
  ...
  // replace
  std::string
  replace(const std::string& in, const std::string& from, const std::string& to);

  // split
  std::vector<std::string>
  split(const std::string& in, const std::string& sep);

  // read lines from a file
  std::vector<std::string>
  readLines(const std::string& fname, const std::string& caller);

  // read all from a file
  std::string
  readAll(const std::string& fname, const std::string& caller);
  ...
  // filled string
  std::string
  filled(int length, char fill = ' ');

  // ****** title ******
  std::string
  boxTitle(const std::string& title, char fill = fmt::gBoxTitleFill, int length = fmt::gBoxWidth);
  // subtitle___________
  std::string
  boxSubTitle(int indent,
      const std::string& subTitle,
      char fill = fmt::gBoxSubTitleFill,
      int length = fmt::gBoxWidth);
  // *******************
  std::string
  boxBaseline(char fill = fmt::gBoxTitleFill, int length = fmt::gBoxWidth);
  ...
  boxEquals(int indent,
      ...
      int precision = fmt::gFloatPrecision) {
    return boxEquals(
        indent, keyWidth, key, std::vector<T>({value}), equalSign, false, nspaces, precision);
  }
  ...
  boxEquals(int indent,
    ...
    for (auto it = keyValue.begin(); it != keyValue.end(); ++it) {
      ss << boxEquals(indent, keyWidth, it->first, it->second, equalSign, nspaces, precision);
    }
    return ss.str();
  }
