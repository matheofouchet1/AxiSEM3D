...
std::vector<double>
SimpleTimer::elapsedAll() const {
  std::vector<double> elapsed;
  int npairs = (int)mTimePoints.size() / 2;
  elapsed.reserve(npairs + 1);
  for (int ipair = 0; ipair < npairs; ipair++) {
    double elap =
        (duration_cast<duration<double>>(mTimePoints[2 * ipair + 1] - mTimePoints[2 * ipair]))
            .count();
    elapsed.push_back(elap);
  }
  if (running()) {
    double elap =
        (duration_cast<duration<double>>(high_resolution_clock::now() - mTimePoints.back()))
            .count();
    elapsed.push_back(elap);
  }
  return elapsed;
...
double
SimpleTimer::elapsedTotal() const {
...
// clock resolution
double
SimpleTimer::getClockResolution() {
  SimpleTimer cpu;
...
MultilevelTimer::MultilevelTimer(int nLevels) :
    mTimers(std::vector<SimpleTimer>(nLevels, SimpleTimer())), mFile(), mCurrentLevel(0) {
  // nothing
}

// activate
void
MultilevelTimer::activate(const std::string& fileName) {
  if (mpi::root()) {
    // deactivate existing
    deactivate();
    // file
...
// begin a sub-process
void
MultilevelTimer::begin(const std::string& procName, char indentFill) {
  if (isActivated()) {
...
void
MultilevelTimer::message(const std::string& what, char indentFill) {
  if (isActivated()) {
    mFile << bstring::filled(mCurrentLevel * 4, indentFill);
    mFile << "<MESSG>  " << what << "\n";
    mFile.flush();
...
// end a sub-process
void
MultilevelTimer::ended(const std::string& procName, char indentFill) {
  if (isActivated()) {
    mCurrentLevel--;
...
double
MultilevelTimer::elapsedCurrentLevel() const {
...
namespace timer {
  ...
  // setup
  void
  setupPreloopDiagnosis(bool activate) {
    if (activate) {
      gPreloopTimer.activate(io::gOutputDirectory + "/develop/preloop_diagnosis.log");
    }
  }
