...
namespace eigen_tools {
  ...
  std::string
  memoryInfo(
      const EigenMat& mat, const std::string& title, double& addToMemGB = useless, int count = 1) {
    // type name
    const std::string& tname = bstring::typeName<typename EigenMat::Scalar>();
    // scalar size
  ...
  std::string
  memoryInfoTensor(const EigenTensor& tensor,
      const std::string& title,
      double& addToMemGB = useless,
      int count = 1) {
    // type name
    const std::string& tname = bstring::typeName<typename EigenTensor::Scalar>();
    // scalar size
  ...
  void
  computeTwoExpIAlphaPhi(int nu_1, double phi, CColX& twoExpIAlphaPhi) {
    if (nu_1 <= 0) {
      // nothing
      return;
    }
  ...
  void
  computeFourierAtPhiExp(const CMatXM& coeffs,
      int nu_1,
      const CColX& twoExpIAlphaPhi,
      RRowM& reals,
      int realRow = 0,
      int ncol = -1) {
    if (ncol < 0) {
    ...
    reals.block(realRow, 0, 1, ncol) =
        (twoExpIAlphaPhi.topRows(nu_1).asDiagonal() * coeffs.block(0, 0, nu_1, ncol))
            .real()
            .colwise()
            .sum();
  }
  ...
  void
  computeFourierAtPhi(
      const CMatXM& coeffs, int nu_1, double phi, RRowM& reals, int realRow = 0, int ncol = -1) {
    // 2 * exp(I * alpha * phi)
    CColX twoExpIAlphaPhi(nu_1);
    computeTwoExpIAlphaPhi(nu_1, phi, twoExpIAlphaPhi);
    // sum over Fourier orders
    computeFourierAtPhiExp(coeffs, nu_1, twoExpIAlphaPhi, reals, realRow, ncol);
  }
} // namespace eigen_tools
