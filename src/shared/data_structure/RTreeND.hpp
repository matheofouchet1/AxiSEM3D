...
// T: type of variables
template <int D, int V, typename T> class RTreeND {
  ///////////// typedef /////////////
  // coordinates
  ...
  // location
  typedef boost::geometry::model::point<double, D, boost::geometry::cs::cartesian> RTreeLoc;
  // leaf
  ...
  static typename std::enable_if<R == 1, RTreeLoc>::type
  toRTreeLoc(const ArrayCS& loc) {
    return RTreeLoc(loc[0]);
  }
  // 2D
  template <int R = D, typename ArrayCS>
  static typename std::enable_if<R == 2, RTreeLoc>::type
  toRTreeLoc(const ArrayCS& loc) {
    return RTreeLoc(loc[0], loc[1]);
  }
  // 3D
  template <int R = D, typename ArrayCS>
  static typename std::enable_if<R == 3, RTreeLoc>::type
  toRTreeLoc(const ArrayCS& loc) {
  ...
  // constructor
  RTreeND(const std::string& ncFile,
      const std::string& coordVarName,
    ...
    timer::gPreloopTimer.message(eigen_tools::memoryInfo(ctrlVals, "values at control points"));
  ...
  void
  addLeaf(const ArrayCS& loc, const TRowV& val) {
  ...
  // add leafs
  void
  addLeafs(const DMatXD_RM& locs, const TMatXV_RM& vals) {
  ...
  template <typename ArrayCS>
  void
  query(const ArrayCS& loc, int count, std::vector<double>& dists, std::vector<TRowV>& vals) const {
    // location
    const RTreeLoc& rloc = toRTreeLoc(loc);
    // KNN query
    std::vector<RTreeLeaf> leafs;
    mRTree.query(boost::geometry::index::nearest(rloc, count), std::back_inserter(leafs));
    // get distance
    dists.clear();
    dists.reserve(count);
    std::transform(
        leafs.begin(), leafs.end(), std::back_inserter(dists), [&rloc](const auto& leaf) {
          return boost::geometry::distance(leaf.first, rloc);
        });
    // get values
    vals.clear();
    vals.reserve(count);
    std::transform(leafs.begin(), leafs.end(), std::back_inserter(vals), [](const auto& leaf) {
      return leaf.second;
    });
  }
  ...
  template <typename ArrayCS>
  TRowV
  compute(const ArrayCS& loc,
      int count,
      double maxDistInRange,
      const TRowV& valOutOfRange,
      double distTolExact) const {
    // query
  ...
  getAllValues() const {
    ...
    std::transform(mRTree.begin(), mRTree.end(), std::back_inserter(vals), [](const auto& leaf) {
      return leaf.second;
    });
    // cast to matrix
  ...
  private:
  // rtree
  boost::geometry::index::rtree<RTreeLeaf, boost::geometry::index::quadratic<16>> mRTree;
};
