...
class NetCDF_Reader {
  ...
  int
  getVariableID(const std::string& vname, const Container& val, bool checkDateType = true) const {
    if (checkDateType) {
      return netcdf::varID_CheckType(mFileID, vname, mFileName, netcdf::typeNC_V(val));
    } else {
  ...
  // get dimensions
  void
  getVariableDimensions(int varid, std::vector<numerical::Int>& dims) const {
    // get number of dimensions
    int ndims = -1;
    netcdf::error(nc_inq_varndims(mFileID, varid, &ndims), "nc_inq_varndims", mFileName);
    dims.resize(ndims);

    // get id of dimensions
    std::vector<int> id_dims(ndims, -1);
    netcdf::error(nc_inq_vardimid(mFileID, varid, id_dims.data()), "nc_inq_vardimid", mFileName);

    // get dimensions
    for (int idim = 0; idim < ndims; idim++) {
      netcdf::error(nc_inq_dimlen(mFileID, id_dims[idim], (size_t*)(&dims[idim])),
          "nc_inq_dimlen",
          mFileName);
    }
  }
  ...
  readVariable(int varid,
    ...
    if (nc_get_vara(mFileID, varid, (size_t*)starts.data(), (size_t*)counts.data(), val.data()) !=
        NC_NOERR) {
  ...
  void
  readMatrix(const std::string& vname, Eigen::DenseBase<Derived>& mat) const {
    ...
    if ((rowsFix == Eigen::Dynamic || rowsFix == dims[0]) &&
      ...
      netcdf::error(nc_get_var(mFileID, varid, mat.derived().data()), "nc_get_var", mFileName);
    } else {
      throw std::runtime_error("NetCDF_Reader::readMatrix || "
  ...
  template <class T>
  void
  readVector(const std::string& vname, std::vector<T>& vec) const {
    // access variable with datatype check
    ...
    // allocate and read
    vec.resize(dims[0]);
    netcdf::error(nc_get_var(mFileID, varid, vec.data()), "nc_get_var", mFileName);
  }

  // Eigen::Tensor
  template <class T, int R>
  void
  readTensor(const std::string& vname, Eigen::Tensor<T, R, Eigen::RowMajor>& tensor) const {
    // access variable with datatype check
    ...
    netcdf::error(nc_get_var(mFileID, varid, tensor.data()), "nc_get_var", mFileName);
  }

  // read string
  void
  readString(const std::string& vname, std::vector<std::string>& dest) const;

  ///////////// read with enforced type of double /////////////
  // Eigen::Matrix
  template <class Derived, typename T = typename Eigen::DenseBase<Derived>::Scalar>
  typename std::enable_if<std::is_floating_point<T>::value && sizeof(T) == sizeof(double),
      void>::type
  readMatrixDouble(const std::string& vname, Eigen::DenseBase<Derived>& mat) const {
    ...
    if ((rowsFix == Eigen::Dynamic || rowsFix == dims[0]) &&
      ...
      mat.derived().resize(dims[0], dims[1]);
      netcdf::error(
          nc_get_var_double(mFileID, varid, mat.derived().data()), "nc_get_var_double", mFileName);
    } else {
      throw std::runtime_error("NetCDF_Reader::readMatrixDouble || "
                               "Inconsistent variable dimensions. || "
                               "Variable name: " +
  ...
  // std::vector
  void
  readVectorDouble(const std::string& vname, std::vector<double>& vec) const {
    // access variable without datatype check
    ...
    // allocate and read
    vec.resize(dims[0]);
    netcdf::error(nc_get_var_double(mFileID, varid, vec.data()), "nc_get_var_double", mFileName);
  }
  ...
  void
  readTensorDouble(
      const std::string& vname, Eigen::Tensor<double, R, Eigen::RowMajor>& tensor) const {
    // access variable without datatype check
    ...
    std::copy(dims.begin(), dims.end(), dimsArray.begin());
    tensor.resize(dimsArray);
    netcdf::error(nc_get_var_double(mFileID, varid, tensor.data()), "nc_get_var_double", mFileName);
  }
