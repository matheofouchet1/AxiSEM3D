...
class ElementOutput {
  ...
  ElementOutput(const std::string& groupName,
      ...
      int bufferSize,
      bool flush) :
      mGroupName(groupName), mMinR(minR), mMaxR(maxR), mMinZ(minZ), mMaxZ(maxZ), mEdgeDim(edgeDim),
      mEdgeCoord(edgeCoord), mIPols(ipols), mPhis(phis), mLats(lats), mLons(lons),
      mNaSpace(naSpace), mWCS(wcs), mFluid(fluid), mUserChannels(userChannels),
      mSamplingPeriod(samplingPeriod), mTmin(tmin), mTmax(tmax), mBufferSize(bufferSize),
      mFlush(flush) {
    // nothing
  }
  ...
  // verbose
  std::string
  verbose(double dt, int numRecordSteps, int npnts, int nphis, int numElements) const;

  public:
  // release ElementOp to domain
  static void
  release(const SE_Model& sem,
      Domain& domain,
      double dt,
      double tmin_simu,
      double tmax_simu,
