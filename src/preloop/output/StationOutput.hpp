...
class StationOutput {
  ...
  StationOutput(const std::string& groupName,
      ...
      int bufferSize,
      bool flush) :
      mGroupName(groupName), mFileName(fileName), mSourceCentered(sourceCentered), mXY(xy),
      mEllipticity(ellipticity), mUseDepth(useDepth), mDepthSolid(depthSolid),
      mUndulatedGeometry(undulatedGeometry), mWCS(wcs), mFluid(fluid), mUserChannels(userChannels),
      mSamplingPeriod(samplingPeriod), mTmin(tmin), mTmax(tmax), mFormat(format),
