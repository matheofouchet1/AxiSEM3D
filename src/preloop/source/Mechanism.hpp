...
class Mechanism {
  ...
  Mechanism(const SM_Type& type, const eigen::DColX& data) : mType(type), mData(data) {
    ...
    if (sSM_TypeDim.at(mType) != mData.size()) {
      throw std::runtime_error("Mechanism::Mechanism || "
          ...
          sSM_TypeStr.at(mType) +
          ": || "
          "Required = " +
          bstring::toString(sSM_TypeDim.at(mType)) +
          " || "
          "Provided = " +
          bstring::toString(mData.size()));
    }
  }
  ...
  // verbose
  std::string
  verbose(int keyWidth) const {
    std::stringstream ss;
    ss << bstring::boxSubTitle(2, "Source mechanism");
    ss << bstring::boxEquals(4,
        keyWidth,
        sSM_TypeStr.at(mType),
        std::vector<double>(mData.data(), mData.data() + mData.size()));
    return ss.str();
  }
