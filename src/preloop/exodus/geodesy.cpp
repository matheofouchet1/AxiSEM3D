...
namespace geodesy {
  ...
  setup(ExodusMesh& exMesh) {
    ...
    internal::iOuterFlattening =
        inparam::gInparamModel.getWithOptions<double>("geodesy:flattening_on_surface",
            {{"SPHERE", 0.},
                {"WGS84", 1. / 298.257223563},
                {"GRS80", 1. / 298.257222101},
                {"SPECFEM3D_GLOBE", 1. / 299.8}});
    // no ellipticity for Cartesian
    if (internal::iOuterFlattening < dEpsilon || internal::iCartesian) {
    ...
    if (mpi::root()) {
      ...
      for (int col = 0; col < ellip.cols(); col++) {
        ...
        internal::iEllipF[col] = ellip(1, col) * internal::iOuterFlattening;
      }
    }
    ...
    ////////////////// geographic //////////////////
    // (lat, lon, r) on positive z-axis
    const std::string& geographicZ =
        inparam::gInparamModel.get<std::string>("geodesy:lat_lon_north_pole_mesh");
    if (geographicZ == "SOURCE") {
      // read from inparam.source.yaml
      const std::string& sourceName =
          inparam::gInparamSource.get<std::string>("list_of_sources:{0}");
      const std::string& rootl = ("list_of_sources:[0]:" + sourceName + ":location");
      try {
        internal::iLatLonRadiusAxisZ(0) =
            inparam::gInparamSource.getWithBounds(rootl + ":latitude_longitude:[0]", -90., 90.);
        internal::iLatLonRadiusAxisZ(1) =
            inparam::gInparamSource.getWithBounds(rootl + ":latitude_longitude:[1]", -180., 180.);
      } catch (...) {
        throw std::runtime_error("geodesy::setup || "
      ...
      if (inparam::gInparamSource.contains(rootl + ":depth")) {
        double depth =
            inparam::gInparamSource.getWithBounds(rootl + ":depth", 0., getOuterRadius());
        if (inparam::gInparamSource.get<bool>(rootl + ":depth_below_solid_surface")) {
          internal::iLatLonRadiusAxisZ(2) = getOuterSolidRadius() - depth;
        } else {
          internal::iLatLonRadiusAxisZ(2) = getOuterRadius() - depth;
        }
      } else {
        internal::iLatLonRadiusAxisZ(2) =
            inparam::gInparamSource.getWithBounds(rootl + ":radius", 0., getOuterRadius());
      }
    } else {
      // read numbers
  ...
  verbose(const eigen::DColX& discontinuities) {
    ...
    // Cartesain
    ss << bstring::boxEquals(0, 21, "model in Cartesian", internal::iCartesian);

    // radius
    ss << bstring::boxEquals(0, 21, "surface radius", internal::iOuterRadius);
    ss << bstring::boxEquals(0, 21, "solid surface radius", internal::iOuterSolidRadius);
    ss << bstring::boxEquals(0, 21, "bottom radius", internal::iInnerRadius);

    // (lat, lon, r) on positive z-axis
    ss << "a reference geographic location on the positive z-axis\n";
    ss << bstring::boxEquals(2, 19, "latitude", internal::iLatLonRadiusAxisZ[0]);
    ss << bstring::boxEquals(2, 19, "longitude", internal::iLatLonRadiusAxisZ[1]);
    ss << bstring::boxEquals(2, 19, "radius", internal::iLatLonRadiusAxisZ[2]);
    ...
    if (internal::iCartesian) {
      ...
      if (internal::iOuterFlattening > numerical::dEpsilon) {
        ss << bstring::boxEquals(0, 21, "flattening on surface", internal::iOuterFlattening);
        ...
        for (int idisc = (int)fDisc.size() - 1; idisc >= 0; idisc--) {
          ss << bstring::boxEquals(
              2, width + 6, "f = " + bstring::toString(fDisc[idisc]), discontinuities[idisc], "at");
        }
