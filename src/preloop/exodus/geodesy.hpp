...
namespace geodesy {
    ...
    for (int ir = 0; ir < r.size(); ir++) {
      try {
        vector_tools::linearInterpSorted(
            internal::iEllipR, r[ir], index0, index1, factor0, factor1);
        f[ir] = (internal::iEllipF[index0] * factor0 + internal::iEllipF[index1] * factor1);
      } catch (...) {
  ...
  Mat
  sz2rtheta(
      const Mat& sz, bool sInFirstColumn, int ids = 0, int idz = 1, int idr = 0, int idt = 1) {
    // numeric
    ...
    if (sInFirstColumn) {
      ...
      rt.col(idr) = (sz.col(ids).array().square() + sz.col(idz).array().square()).sqrt();
      rt.col(idt) =
          (rt.col(idr).array() < epsilon)
              .select(
                  (T)0., (sz.col(idz).cwiseQuotient(rt.col(idr).cwiseMax(epsilon)).array().acos()));
    } else {
      // s in the first row
      rt.row(idr) = (sz.row(ids).array().square() + sz.row(idz).array().square()).sqrt();
      rt.row(idt) =
          (rt.row(idr).array() < epsilon)
              .select(
                  (T)0., (sz.row(idz).cwiseQuotient(rt.row(idr).cwiseMax(epsilon)).array().acos()));
    }
    return rt;
  }
  ...
  xy2sphi(const Mat& xy,
    ...
    if (xInFirstColumn) {
      ...
      sp.col(ids) = (xy.col(idx).array().square() + xy.col(idy).array().square()).sqrt();
      sp.col(idp) = xy.col(idx).binaryExpr(xy.col(idy), lambda_atan2);
    } else {
      // x in the first row
      sp.row(ids) = (xy.row(idx).array().square() + xy.row(idy).array().square()).sqrt();
  ...
  template <typename Mat, typename T = typename Mat::Scalar>
  Mat
  tpr2llr(const Mat& tpr, bool ellipticity) {
  ...
  template <typename Mat, typename T = typename Mat::Scalar>
  Mat
  llr2tpr(const Mat& llr, bool ellipticity) {
    static const T halfPi = (T)(numerical::dPi / 2.);
    static const T degree = (T)(numerical::dDegree);
    static const T one = (T)(1.);
  ...
  template <typename Mat, typename T = typename Mat::Scalar>
  Mat
  spz2llr(const Mat& spzSrc, bool ellipticity, bool lon360) {
    // (s, phi, z) -> (x, y, z)
    ...
    spzGeo.col(1) = ((spzGeo.col(1).array() < err * .9).select(spzSrc.col(1), spzGeo.col(1)));
    ...
    llr.col(2) = (spzSrc.col(0).array().square() + spzSrc.col(2).array().square()).sqrt();
    // longitude range
    if (lon360) {
      // 0 ~ 360
      llr.col(1) =
          (llr.col(1).array() < 0.).select(llr.col(1).array() + (T)360., llr.col(1).array());
    } else {
      // -180 ~ 180
      llr.col(1) =
          (llr.col(1).array() > 180.).select(llr.col(1).array() - (T)360., llr.col(1).array());
    }
    return llr;
  }
  ...
  template <typename Mat, typename T = typename Mat::Scalar>
  Mat
  llr2spz(const Mat& llr, bool ellipticity) {
    // (lat, lon, r) -> (t, p, r)
    ...
    xyzGeo.col(0) =
        (tprGeo.col(2).array() * tprGeo.col(0).array().sin() * tprGeo.col(1).array().cos());
    xyzGeo.col(1) =
        (tprGeo.col(2).array() * tprGeo.col(0).array().sin() * tprGeo.col(1).array().sin());
    ...
    spzSrc.col(1) = ((spzSrc.col(1).array() < err * .9).select(tprGeo.col(1), spzSrc.col(1)));
  ...
  template <typename Mat,
      typename T = typename Mat::Scalar,
  ...
  backAzimuth(const Mat& llr, bool ellipticity) {
    ...
    xy.col(0) =
        ((a - gg.array()).square() + (b - hh.array()).square() + (c - ff.array()).square() - (T)2.);
    xy.col(1) = ((a - dd.array()).square() + (b - ee.array()).square() + c * c - (T)2.);
