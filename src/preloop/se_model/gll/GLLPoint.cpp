...
// release to domain
void
GLLPoint::release(const ABC& abc, const TimeScheme& timeScheme, Domain& domain) {
  //////////////////////////// reduce ////////////////////////////
  ...
  if (mMassSolid.rows() > 0) {
    ...
    // point
    mSolidPoint =
        std::make_shared<SolidPoint>(mNr, mCoords.transpose(), mGlobalTag, mass, timeScheme);
    // release
  ...
  if (mMassFluid.rows() > 0) {
    ...
    // point
    mFluidPoint =
        std::make_shared<FluidPoint>(mNr, mCoords.transpose(), mGlobalTag, mass, timeScheme);
    // release
  ...
  if (mSolidPoint && mFluidPoint) {
    ...
    if (mNormalSFA.rows() == 1 && mMassFluid.rows() == 1) {
      // 1D coupling
      sfc = std::make_unique<SolidFluidCoupling1D>(mSolidPoint,
          mFluidPoint,
          mNormalSFU(0, 0),
          mNormalSFU(0, 2),
          mNormalSFA(0, 0),
      ...
      // 3D coupling
      sfc = std::make_unique<SolidFluidCoupling3D>(mSolidPoint,
          mFluidPoint,
          op1D_3D::to3D(mNormalSFU, mNr),
          op1D_3D::to3D(mNormalSFA, mNr),
    ...
    for (auto itv = itm->second.begin(); itv != itm->second.end(); itv++) {
      ...
      if (fluid) {
        ...
        if (nABC.rows() == 1 && rhoVp.rows() == 1) {
          // 1D fluid
          clayton =
              std::make_unique<const ClaytonFluid1D>(mFluidPoint, rhoVp(0), nABC.row(0).norm());
        } else {
          // 3D fluid
          clayton = std::make_unique<const ClaytonFluid3D>(
              mFluidPoint, op1D_3D::to3D(rhoVp, mNr), op1D_3D::to3D(nABC, mNr).rowwise().norm());
        }
  ...
  if (abc.sponge() && mCountGammasAdded > 0) {
    if (mFluidPoint) {
      ...
      if (mGamma.rows() == 1) {
        sponge_f =
            std::make_unique<const Sponge<FluidPoint>>(mFluidPoint, mGamma(0) / mCountGammasAdded);
      } else {
        sponge_f =
            std::make_unique<const Sponge<FluidPoint>>(mFluidPoint, mGamma / mCountGammasAdded);
      }
    ...
    if (mSolidPoint) {
      ...
      if (mGamma.rows() == 1) {
        sponge_s =
            std::make_unique<const Sponge<SolidPoint>>(mSolidPoint, mGamma(0) / mCountGammasAdded);
      } else {
        sponge_s =
            std::make_unique<const Sponge<SolidPoint>>(mSolidPoint, mGamma / mCountGammasAdded);
      }
  ...
  if (mFluidPoint && mSurface &&
      std::find(abc.getBoundaryKeys().begin(), abc.getBoundaryKeys().end(), "TOP") ==
          abc.getBoundaryKeys().end()) {
