...
namespace op1D_3D {
  ...
  void
  times(const MatA& A, const MatB& B, MatR& R) {
    if (A.rows() == B.rows()) {
      // 1Dx1D or 3Dx3D
      R = A.cwiseProduct(B);
    } else if (A.rows() == 1) {
  ...
  template <class Mat>
  bool
  regularize1D(const std::vector<std::reference_wrapper<Mat>>& mats) {
    // find max nr
    int nrMax = -1;
  ...
  tryReduceTo1D(Mat& m) {
    ...
    if ((m.rowwise() - m.row(0)).norm() < m.norm() * numerical::epsilon<T>()) {
