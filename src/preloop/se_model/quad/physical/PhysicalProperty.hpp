...
template <int M> class PhysicalProperty {
  ...
  static void
  linInterpPhi(const MatIn& dataIn, MatOut& dataOut, int colIn, int colOut, int nrIn, int nrOut) {
    if (nrIn == nrOut) {
      ...
      for (int alpha = 0; alpha < nrOut; alpha++) {
        ...
        dataOut(alpha, colOut) = (val0 + (val1 - val0) / dPhiIn * (phi - loc0 * dPhiIn));
      }
    }
  }
...
class NodalPhysicalProperty : public PhysicalProperty<spectral::nPEM> {
  ...
  NodalPhysicalProperty(const NodalPhysicalProperty& other) :
      PhysicalProperty<spectral::nPEM>(other), mNodal(other.mNodal), mAxial(other.mAxial) {
    // nothing
  }
  ...
  // operator +
  NodalPhysicalProperty
  operator+(const NodalPhysicalProperty& other) const {
    // copy this
    NodalPhysicalProperty result(*this);
    // nodal
