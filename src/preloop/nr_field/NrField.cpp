...
// build from inparam
std::unique_ptr<const NrField>
NrField::buildInparam(const ExodusMesh& exodusMesh) {
  // read type and lucky number
  const std::string& type = inparam::gInparamNr.getWithLimits<std::string>("type_Nr",
      {{"CONSTANT", "CONSTANT"},
  ...
  if (type == "CONSTANT") {
    ...
    const std::string& fname = inparam::gInparamNr.get<std::string>("pointwise:nc_data_file");
    const double& factor = inparam::gInparamNr.get<double>("pointwise:multip_factor");
    nrField = std::make_unique<const NrFieldPointwise>(
        fname, factor, exodusMesh.getGlobalVariable("dist_tolerance"));

  } else if (type == "STRUCTURED") {
    const std::string& fname = inparam::gInparamNr.get<std::string>("structured:nc_data_file");
    int valOOR = inparam::gInparamNr.getWithBounds("structured:value_out_of_range", 0);
...
// next lucky number
int
NrField::nextLuckyNumber(int n) {
  auto it = std::lower_bound(sLuckyNumbers.begin(), sLuckyNumbers.end(), n);
...
// lucky numbers
std::vector<int> NrField::sLuckyNumbers = {1,
    2,
    3,
    4,
    5,
