...
// constructor
NrFieldPointwise::NrFieldPointwise(const std::string& fname, double factor, double distTolExact) :
    mFilename(fname), mFactor(factor), mDistTolExact(distTolExact) {
  // rtree
  std::array<std::pair<std::string, double>, 1> varInfo = {std::make_pair("pointwise_Nr", 1.)};
  mRTree = std::make_unique<RTreeND<2, 1, int>>(fname, "pointwise_sz", varInfo);
...
NrFieldPointwise::verbose() const {
  ...
  if (mSumNrStart != -1) {
    ...
    ss << bstring::boxEquals(2, 19, "compression ratio", 1. * sumNr / mSumNrStart);
  }
