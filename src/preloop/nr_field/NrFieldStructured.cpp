...
// constructor
NrFieldStructured::NrFieldStructured(const std::string& fname, int valOutOfRange) :
  ...
  // data
  std::vector<std::pair<std::string, double>> varInfo = {std::make_pair("structured_Nr", 1.)};
...
// verbose
std::string
NrFieldStructured::verbose() const {
  ...
  long sum = ((Eigen::Tensor<long, 0, Eigen::RowMajor>)data.cast<long>().sum())(0);
